...
float * Concate::forward(float *x, float *y) {
    ...
    for(unsigned int i=0;i<B*H*W;++i){
        cudaMemcpy(&output[i*C],&x[i*C1],C1*sizeof(float),cudaMemcpyDeviceToDevice);
    }
    for(unsigned int i=0;i<B*H*W;++i){
        cudaMemcpy(&output[i*C+C1],&y[i*C2],C2*sizeof(float),cudaMemcpyDeviceToDevice);
    }*/
