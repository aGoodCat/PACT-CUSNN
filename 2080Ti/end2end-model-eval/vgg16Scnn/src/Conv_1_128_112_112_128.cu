...
__global__ void conv_1_128_112_112_128_transform(float *matrix, float *d_nnz, char *ids, unsigned int *c_lens){
    ...
    for(int tile_h_id = th_start;tile_h_id>=0;tile_h_id--){
        ...
        for(int tile_w_id = tw_start;tile_w_id>=0;tile_w_id--){
            ...
            unsigned int index = atomicAdd(&c_lens[batch_id*128*608+c*608+tile_id],1);
            unsigned int abs_h = h - tile_h_id*3;
            unsigned int abs_w = w - tile_w_id*7;
            d_nnz[batch_id*128*608*(3+3-1)*(7+3-1)+
                  c*608*(3+3-1)*(7+3-1)+tile_id*(3+3-1)*(7+3-1)+index] = v;
            ids[batch_id*128*608*(3+3-1)*(7+3-1)+
                c*608*(3+3-1)*(7+3-1)+tile_id*(3+3-1)*(7+3-1)+index] = abs_h*(7+3-1)+abs_w;
        }
    }
}
...
__device__ void conv_1_128_112_112_128_load_input_2_shared_memory(float *values,unsigned int *c_lens,char *ids,
    ...
    for(unsigned int c_id=warp_id;c_id<16&&tile_c_id+c_id<128;c_id+=4){
        unsigned int end_index = c_lens[batch_id*128*608+(tile_c_id+c_id)*608+tile_id];
        if(lane_id ==0){
            shared_lens[c_id] = end_index;
        }
        for(unsigned int id = lane_id;id<end_index;id+=32){
            shared_input[c_id*(3+3-1)*(7+3-1)+id] = values[batch_id*128*608*(3+3-1)*(7+3-1)+
                                                           (tile_c_id+c_id)*608*(3+3-1)*(7+3-1)+tile_id*(3+3-1)*(7+3-1)+id];
            shared_ids[c_id*(3+3-1)*(7+3-1)+id] = ids[batch_id*128*608*(3+3-1)*(7+3-1)+
                                                      (tile_c_id+c_id)*608*(3+3-1)*(7+3-1)+tile_id*(3+3-1)*(7+3-1)+id];
        }
    }
}
__global__ void conv_1_128_112_112_128_conv2d(float * __restrict__ values, unsigned int * __restrict__ c_lens,
    ...
    const unsigned int batch_id = (blockIdx.x/(8*608));
    const unsigned int t_id = (blockIdx.x - batch_id*8*608)/8;
    const unsigned int tile_h_id = (t_id / 16)*3;
    ...
    for(unsigned int n = threadIdx.x;n<128;n+=128){
        for(unsigned int c=start_channel_index;c<start_channel_index+16&&c<128;c++){
