...
__global__ void conv_1_512_28_28_512_transform(float *matrix, float *d_nnz, char *ids, unsigned int *c_lens){
    ...
    for(int tile_h_id = th_start;tile_h_id>=0;tile_h_id--){
        ...
        for(int tile_w_id = tw_start;tile_w_id>=0;tile_w_id--){
            ...
            unsigned int index = atomicAdd(&c_lens[batch_id*512*28+c*28+tile_id],1);
            unsigned int abs_h = h - tile_h_id*4;
            unsigned int abs_w = w - tile_w_id*7;
            d_nnz[batch_id*512*28*(4+3-1)*(7+3-1)+
                  c*28*(4+3-1)*(7+3-1)+tile_id*(4+3-1)*(7+3-1)+index] = v;
            ids[batch_id*512*28*(4+3-1)*(7+3-1)+
...
__device__ void conv_1_512_28_28_512_load_input_2_shared_memory(float *values,unsigned int *c_lens,char *ids,
    ...
    for(unsigned int c_id=warp_id;c_id<4&&tile_c_id+c_id<512;c_id+=16){
        unsigned int end_index = c_lens[batch_id*512*28+(tile_c_id+c_id)*28+tile_id];
        if(lane_id ==0){
            shared_lens[c_id] = end_index;
        }
        for(unsigned int id = lane_id;id<end_index;id+=32){
            shared_input[c_id*(4+3-1)*(7+3-1)+id] = values[batch_id*512*28*(4+3-1)*(7+3-1)+
                                                           (tile_c_id+c_id)*28*(4+3-1)*(7+3-1)+tile_id*(4+3-1)*(7+3-1)+id];
            shared_ids[c_id*(4+3-1)*(7+3-1)+id] = ids[batch_id*512*28*(4+3-1)*(7+3-1)+
                                                      (tile_c_id+c_id)*28*(4+3-1)*(7+3-1)+tile_id*(4+3-1)*(7+3-1)+id];
        }
    }
}
__global__ void conv_1_512_28_28_512_conv2d(float * __restrict__ values, unsigned int * __restrict__ c_lens,
    ...
    __shared__ float input[4*(4+3-1)*(7+3-1)];
    __shared__ char input_ids[4*(4+3-1)*(7+3-1)];
    __shared__ unsigned int channel_lens[(4)];

    const unsigned int batch_id = (blockIdx.x/(128*28));
    const unsigned int t_id = (blockIdx.x - batch_id*128*28)/128;
    const unsigned int tile_h_id = (t_id / 4)*4;
    const unsigned int tile_w_id = (t_id % 4)*7;
    const unsigned int index = blockIdx.x % (128);
    ...
    for(unsigned int n = threadIdx.x;n<512;n+=512){
        for(unsigned int c=start_channel_index;c<start_channel_index+4&&c<512;c++){
