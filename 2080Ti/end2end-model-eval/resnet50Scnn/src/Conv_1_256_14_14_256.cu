...
__global__ void conv_1_256_14_14_256_transform(float *matrix, float *d_nnz, char *ids, unsigned int *c_lens){
    ...
    for(int tile_h_id = th_start;tile_h_id>=0;tile_h_id--){
        ...
        for(int tile_w_id = tw_start;tile_w_id>=0;tile_w_id--){
            ...
            unsigned int index = atomicAdd(&c_lens[batch_id*256*14+c*14+tile_id],1);
            unsigned int abs_h = h - tile_h_id*2;
            unsigned int abs_w = w - tile_w_id*7;
            d_nnz[batch_id*256*14*(2+3-1)*(7+3-1)+
                  c*14*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+index] = v;
            ids[batch_id*256*14*(2+3-1)*(7+3-1)+
...
__device__ void conv_1_256_14_14_256_load_input_2_shared_memory(float *values,unsigned int *c_lens,char *ids,
    ...
    for(unsigned int c_id=warp_id;c_id<16&&tile_c_id+c_id<256;c_id+=8){
        unsigned int end_index = c_lens[batch_id*256*14+(tile_c_id+c_id)*14+tile_id];
        if(lane_id ==0){
            shared_lens[c_id] = end_index;
        }
        for(unsigned int id = lane_id;id<end_index;id+=32){
            shared_input[c_id*(2+3-1)*(7+3-1)+id] = values[batch_id*256*14*(2+3-1)*(7+3-1)+
                                                           (tile_c_id+c_id)*14*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+id];
            shared_ids[c_id*(2+3-1)*(7+3-1)+id] = ids[batch_id*256*14*(2+3-1)*(7+3-1)+
                                                      (tile_c_id+c_id)*14*(2+3-1)*(7+3-1)+tile_id*(2+3-1)*(7+3-1)+id];
        }
    }
}
__global__ void conv_1_256_14_14_256_conv2d(float * __restrict__ values, unsigned int * __restrict__ c_lens,
    ...
    const unsigned int batch_id = (blockIdx.x/(16*14));
    const unsigned int t_id = (blockIdx.x - batch_id*16*14)/16;
    ...
    for(unsigned int n = threadIdx.x;n<256;n+=256){
        for(unsigned int c=start_channel_index;c<start_channel_index+16&&c<256;c++){
...
void Conv_1_256_14_14_256::initialize(unsigned int b,unsigned int c,unsigned int h,unsigned int w,unsigned int n,
...
float * Conv_1_256_14_14_256::forward(float *input) {
    cudaMemset(output, 0, B*N*hOut*wOut*sizeof(float));
    cudaMemset(c_lens, 0, B*C*TILES_EACH_CHANNEL*sizeof(unsigned int));
    conv_1_256_14_14_256_transform<<<(B*C*H*W-1)/512+1,512>>>(input,this->nnz,this->ids,this->c_lens);
    conv_1_256_14_14_256_conv2d<<<B*TCS*TILES_EACH_CHANNEL,N>>>(this->nnz,this->c_lens,this->ids,this->kernel,this->output);
