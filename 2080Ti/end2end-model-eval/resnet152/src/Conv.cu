...
/*int main(void){
    Conv conv(1,3,224,224,64,3,7,7,2,"../weights/conv1_conv_0.bin","../weights/conv1_conv_1.bin");
    float *input;
    float *hostInput = (float *)malloc((1*3*224*224)*sizeof(float));
    for(int i=0;i<1*3*224*224;++i){
        hostInput[i] = 1.0f;
    }
    cudaMalloc(&input,1*3*224*224*sizeof(float));
    cudaMemcpy(input,hostInput,1*3*224*224*sizeof(float),cudaMemcpyHostToDevice);
    //conv.forward(input);
    float *outputPython = load_input("../conv.bin",1*112*112*64);
    float *outputCudnn = (float *)malloc(112*112*64*sizeof(float));
    cudaMemcpy(outputCudnn,conv.forward(input),112*112*64*sizeof(float),cudaMemcpyDeviceToHost);
    float diff = 0.0f;
