...
__global__ void conv_1_128_14_14_32_transform(float *matrix, float *d_nnz, char *ids, unsigned int *c_lens){
    ...
    unsigned int batch_id = global_id/(128*14*14);
    if(global_id >= 1*14*14*128){
        return ;
    }
    ...
    int h = ((global_id - batch_id * 128 * 14 * 14)/128)/14+1;
    int w = ((global_id - batch_id * 128 * 14 * 14)/128)%14+1;
    int th_start = min(h/3,5-1);
    int tw_start = min(w/7,2-1);
    for(int tile_h_id = th_start;tile_h_id>=0;tile_h_id--){
        if((tile_h_id*3+3+3-1)<=h){
            break;
        }
        for(int tile_w_id = tw_start;tile_w_id>=0;tile_w_id--){
            ...
            unsigned int index = atomicAdd(&c_lens[batch_id*128*10+c*10+tile_id],1);
            unsigned int abs_h = h - tile_h_id*3;
            unsigned int abs_w = w - tile_w_id*7;
            d_nnz[batch_id*128*10*(3+3-1)*(7+3-1)+
                  c*10*(3+3-1)*(7+3-1)+tile_id*(3+3-1)*(7+3-1)+index] = v;
            ids[batch_id*128*10*(3+3-1)*(7+3-1)+
                c*10*(3+3-1)*(7+3-1)+tile_id*(3+3-1)*(7+3-1)+index] = abs_h*(7+3-1)+abs_w;
        }
    }
}
...
__device__ void conv_1_128_14_14_32_load_input_2_shared_memory(float *values,unsigned int *c_lens,char *ids,
    ...
    for(unsigned int c_id=warp_id;c_id<4&&tile_c_id+c_id<128;c_id+=1){
        unsigned int end_index = c_lens[batch_id*128*10+(tile_c_id+c_id)*10+tile_id];
        if(lane_id ==0){
            shared_lens[c_id] = end_index;
        }
        for(unsigned int id = lane_id;id<end_index;id+=32){
            shared_input[c_id*(3+3-1)*(7+3-1)+id] = values[batch_id*128*10*(3+3-1)*(7+3-1)+
                                                           (tile_c_id+c_id)*10*(3+3-1)*(7+3-1)+tile_id*(3+3-1)*(7+3-1)+id];
            shared_ids[c_id*(3+3-1)*(7+3-1)+id] = ids[batch_id*128*10*(3+3-1)*(7+3-1)+
                                                      (tile_c_id+c_id)*10*(3+3-1)*(7+3-1)+tile_id*(3+3-1)*(7+3-1)+id];
        }
    }
}
__global__ void conv_1_128_14_14_32_conv2d(float * __restrict__ values, unsigned int * __restrict__ c_lens,
    ...
    __shared__ float input[4*(3+3-1)*(7+3-1)];
    __shared__ char input_ids[4*(3+3-1)*(7+3-1)];
    __shared__ unsigned int channel_lens[(4)];

    const unsigned int batch_id = (blockIdx.x/(32*10));
    const unsigned int t_id = (blockIdx.x - batch_id*32*10)/32;
    const unsigned int tile_h_id = (t_id / 2)*3;
    const unsigned int tile_w_id = (t_id % 2)*7;
    const unsigned int index = blockIdx.x % (32);
    const unsigned int start_channel_index = index*4;
    const unsigned int warp_id = threadIdx.x / 32;
    const unsigned int lane_id = threadIdx.x % 32;
    float data_array[9];
    float temp_result[3*7] = {0.0f};
    conv_1_128_14_14_32_load_input_2_shared_memory(values,c_lens,ids,input,input_ids,channel_lens,warp_id,lane_id,batch_id,t_id,start_channel_index);
    __syncthreads();
    float v;
    unsigned int id;
    for(unsigned int n = threadIdx.x;n<32;n+=32){
        for(unsigned int c=start_channel_index;c<start_channel_index+4&&c<128;c++){
            unsigned int abs_c = c - start_channel_index;
            unsigned int start_index = abs_c*(3+3-1)*(7+3-1);
            unsigned int end_index = start_index+channel_lens[abs_c];
            if(start_index == end_index){
                continue;
            }
            conv_1_128_14_14_32_load_data_2_register(data_array,(c),kernel,n);
            ...
            for(unsigned int iter=0;iter<iters;iter++) {
                ...
                conv_1_128_14_14_32_switch_function(id,data_array,v,temp_result);
            }
        }
        for (unsigned int th = 0; th < 3; ++th) {
            for (unsigned int tw = 0; tw < 7; ++tw) {
                ...
                atomicAdd(&outputs[batch_id * 32 * 14 * 14 + (tile_h_id + th) * 14 * 32 + (tile_w_id + tw) * 32 +
                                   n],temp_result[(th * 7 + tw)]);
            }
        }
        for(unsigned int i=0;i<3*7;++i){
            temp_result[i] = 0.0f;
        }
    }
}
void Conv_1_128_14_14_32::initialize(unsigned int b,unsigned int c,unsigned int h,unsigned int w,unsigned int n,
...
float * Conv_1_128_14_14_32::forward(float *input) {
    cudaMemset(output, 0, B*N*hOut*wOut*sizeof(float));
    cudaMemset(c_lens, 0, B*C*TILES_EACH_CHANNEL*sizeof(unsigned int));
    conv_1_128_14_14_32_transform<<<(B*C*H*W-1)/512+1,512>>>(input,this->nnz,this->ids,this->c_lens);
    conv_1_128_14_14_32_conv2d<<<B*TCS*TILES_EACH_CHANNEL,N>>>(this->nnz,this->c_lens,this->ids,this->kernel,this->output);
