...
__global__ void conv_1_512_7_7_512_conv2d(float * __restrict__ values, unsigned int * __restrict__ c_lens,
    ...
    const unsigned int batch_id = (blockIdx.x/(128*4));
    const unsigned int t_id = (blockIdx.x - batch_id*128*4)/128;
...
void Conv_1_512_7_7_512::initialize(unsigned int b,unsigned int c,unsigned int h,unsigned int w,unsigned int n,
...
float * Conv_1_512_7_7_512::forward(float *input) {
    cudaMemset(output, 0, B*N*hOut*wOut*sizeof(float));
    cudaMemset(c_lens, 0, B*C*TILES_EACH_CHANNEL*sizeof(unsigned int));
    conv_1_512_7_7_512_transform<<<(B*C*H*W-1)/512+1,512>>>(input,this->nnz,this->ids,this->c_lens);
    conv_1_512_7_7_512_conv2d<<<B*TCS*TILES_EACH_CHANNEL,N>>>(this->nnz,this->c_lens,this->ids,this->kernel,this->output);
