...
__device__ void conv_1_256_56_56_256_load_input_2_shared_memory(float *values,unsigned int *c_lens,char *ids,
    ...
    for(unsigned int c_id=warp_id;c_id<4&&tile_c_id+c_id<256;c_id+=8){
...
__global__ void conv_1_256_56_56_256_conv2d(float * __restrict__ values, unsigned int * __restrict__ c_lens,
    ...
    const unsigned int batch_id = (blockIdx.x/(64*224));
    const unsigned int t_id = (blockIdx.x - batch_id*64*224)/64;
    ...
    for(unsigned int n = threadIdx.x;n<256;n+=256){
        for(unsigned int c=start_channel_index;c<start_channel_index+4&&c<256;c++){
