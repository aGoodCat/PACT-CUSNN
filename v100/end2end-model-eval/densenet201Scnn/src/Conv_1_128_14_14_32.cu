...
__global__ void conv_1_128_14_14_32_conv2d(float * __restrict__ values, unsigned int * __restrict__ c_lens,
    ...
    __shared__ float input[2*(3+3-1)*(7+3-1)];
    __shared__ char input_ids[2*(3+3-1)*(7+3-1)];
    __shared__ unsigned int channel_lens[(2)];

    const unsigned int batch_id = (blockIdx.x/(64*10));
    const unsigned int t_id = (blockIdx.x - batch_id*64*10)/64;
