...
print(save_layers)
net = tf.keras.applications.ResNet50(include_top=True, weights='imagenet',
...
for layer in net.layers:
    ...
    if 'Conv' in layer.name:
        ...
        weight1 = layer.get_weights()[0]
        weight1 = np.asarray(weight1)
        C = len(layer.get_weights())
        for c in range(1,C):
            w = np.asarray(layer.get_weights()[c])
            weight1 = np.concatenate((weight1, w), axis=0)
        weight_file = './weights/{}.bin'.format(layer.name)
        weight1.tofile(weight_file)
    else:
